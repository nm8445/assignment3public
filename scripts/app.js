...
(function(){
    ...
    function CheckLogin() {
        if (sessionStorage.getItem("user")) {
            ...
            $("#eventPlanningNavItem").show();  // Show the Events Planning Page link

            $("#logout").on("click", function() {
                sessionStorage.clear();
                location.href = "login.html";
                $("#eventPlanningNavItem").hide();  // Hide the Events Planning Page link
            });
        } else {
            $("#eventPlanningNavItem").hide();  // Hide the Events Planning Page link
        }
    ...
    function enableEventEditing(paragraphId, buttonId) {
        const editButton = document.getElementById(buttonId);
        const eventParagraph = document.getElementById(paragraphId);

        if (editButton && eventParagraph) {
            editButton.addEventListener('click', () => {
                if (eventParagraph.isContentEditable) {
                    // If already editable, turn off editing and change button text
                    eventParagraph.contentEditable = 'false';
                    editButton.textContent = 'Edit Event';
                } else {
                    // If not editable, turn on editing and change button text
                    eventParagraph.contentEditable = 'true';
                    editButton.textContent = 'Save Changes';
                }
            });
        } else {
            console.error('Element not found:', paragraphId, 'or', buttonId);
        }
    }
    ...
    function Start(){
        ...
        CheckLogin();
        enableEventEditing('eventsParagraph', 'editEventButton');
        enableEventEditing('soccerevent', 'soccereditbutton');
